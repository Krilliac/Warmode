...
namespace Org.BouncyCastle.Crypto.Engines
{
	public class SkipjackEngine : IBlockCipher
	{
		...
		{
			int num = ((int)input[inOff] << 8) + (int)(input[inOff + 1] & 255);
			int num2 = ((int)input[inOff + 2] << 8) + (int)(input[inOff + 3] & 255);
			int num3 = ((int)input[inOff + 4] << 8) + (int)(input[inOff + 5] & 255);
			int num4 = ((int)input[inOff + 6] << 8) + (int)(input[inOff + 7] & 255);
			int num5 = 0;
			for (int i = 0; i < 2; i++)
			{
				for (int j = 0; j < 8; j++)
