...
namespace RootMotion.FinalIK
{
	...
	public class GrounderQuadruped : Grounder
	{
		...
		[Range(0f, 1f), Tooltip("The weight of rotating the character root to the ground angle (range: 0 - 1).")]
		public float rootRotationWeight = 0.5f;

		[Range(-90f, 0f), Tooltip("The maximum angle of rotating the quadruped downwards (going downhill, range: -90 - 0).")]
		public float minRootRotation = -25f;

		[Range(0f, 90f), Tooltip("The maximum angle of rotating the quadruped upwards (going uphill, range: 0 - 90).")]
		...
		[Range(0f, 1f), Tooltip("The weight of maintaining the head's rotation as it was before solving the Grounding (range: 0 - 1).")]
		...
		private Transform[] InitiateFeet(IK[] ikComponents, ref GrounderQuadruped.Foot[] f, int indexOffset)
		{
			...
			for (int i = 0; i < ikComponents.Length; i++)
			{
				...
				IKSolver expr_67 = f[i + indexOffset].solver;
				expr_67.OnPreUpdate = (IKSolver.UpdateDelegate)Delegate.Combine(expr_67.OnPreUpdate, new IKSolver.UpdateDelegate(this.OnSolverUpdate));
				IKSolver expr_97 = f[i + indexOffset].solver;
				expr_97.OnPostUpdate = (IKSolver.UpdateDelegate)Delegate.Combine(expr_97.OnPostUpdate, new IKSolver.UpdateDelegate(this.OnPostSolverUpdate));
			}
			return array;
		}
		...
		private void RootRotation()
		{
			...
			Vector3 vector = Quaternion.Inverse(quaternion) * point;
