...
namespace RootMotion.FinalIK
{
	...
	public class IKConstraintBend
	{
		...
		public Vector3 GetDir()
		{
			...
			{
				Vector3 lhs = this.bendGoal.position - this.node1.solverPosition;
				if (lhs != Vector3.zero)
				{
					this.direction = lhs;
				}
			...
			Vector3 vector = this.node3.solverPosition - this.node1.solverPosition;
			Quaternion rotation = Quaternion.FromToRotation(this.node3.transform.position - this.node1.transform.position, vector);
			Vector3 vector2 = rotation * (this.node2.transform.position - this.node1.transform.position);
			if (this.node3.effectorRotationWeight > 0f)
			{
				Vector3 b = -Vector3.Cross(vector, this.node3.solverRotation * this.defaultChildDirection);
				vector2 = Vector3.Lerp(vector2, b, this.node3.effectorRotationWeight);
			}
			...
			return Vector3.Lerp(vector2, this.direction.normalized, this.weight);
		}
		...
		{
			Vector3 vector = this.node2.solverPosition - this.node1.solverPosition;
			Vector3.OrthoNormalize(ref vector, ref tangent);
			return tangent;
		}
	}
