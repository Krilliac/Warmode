	...
	public class IKSolverFullBodyBiped : IKSolverFullBody
	{
		...
		public override bool IsValid(bool log)
		{
			...
			if (this.rootNode == null)
			{
				if (log)
				{
					base.LogWarning("Root Node bone is null. FBBIK will not initiate.");
				}
				return false;
			}
			if (this.chain.Length != 5 || this.chain[0].nodes.Length != 1 || this.chain[1].nodes.Length != 3 || this.chain[2].nodes.Length != 3 || this.chain[3].nodes.Length != 3 || this.chain[4].nodes.Length != 3 || this.effectors.Length != 9 || this.limbMappings.Length != 4)
			{
				if (log)
				{
					base.LogWarning("Invalid FBIK Biped setup. FBBIK will not initiate.");
				}
				return false;
			}
			return true;
		...
		protected override void ApplyBendConstraints()
		{
			...
			else
			{
				...
				for (int i = 0; i < 5; i++)
				{
					this.effectors[i].GetNode().solverPosition += this.offset;
				}
			}
